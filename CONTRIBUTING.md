...
This outlines how to propose a change to ernest. For a detailed
...
Ernest is in an initially stable state of development, with a great deal
...
- Fork the package and clone onto your computer. If you haven’t done
  ...
  `usethis::create_from_github("kylesnap/ernest", fork = TRUE)`.
...
- Some of ernest’s tests can take a while to run. These tests can be
  skipped by setting the `ERNEST_EXTENDED_TESTS` environment option to
  `false`.

## Code of Conduct

Please note that the ernest project is released with a [Contributor Code
of Conduct](https://kylesnap.github.io/ernest/CODE_OF_CONDUCT.md). By
