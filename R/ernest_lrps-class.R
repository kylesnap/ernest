...
ernest_lrps <- R6Class(
  ...
  public = list(
    ...
    propose_live = function(original, criterion) {
      cli::cli_abort("{.fn propose_live} must be called on a subclass of {.cls propose_live}.")
    },
...
rwcube_lrps <- R6::R6Class(
  ...
  inherit = ernest_lrps,
  public = list(
    initialize = function(log_lik_fn, prior_fn, n_dim, steps = 20L, epsilon = 1) {
    ...
    print = function(...) {
      ...
      cli::cli_dl(c(
        "Current Config." = "{private$steps} steps @ {prettyNum(private$cur_epsilon)} step-size",
