...
new_ernest_likelihood <- function(
  ...
  force(fn)
  structure(
    \(x) catch_nonfinite(vectorized_log_lik(fn, x, interface), on_nonfinite),
    body = fn,
    interface = interface,
    on_nonfinite = on_nonfinite,
...
catch_nonfinite <- function(y, on_nonfinite) {
  ...
  if (length(bad) > 0) {
    switch(
      on_nonfinite,
      "warn" = cli::cli_warn(
        "Replacing log-lik. values with `-Inf`: {unique(y[bad])}",
        call = NULL
      ),
      "abort" = cli::cli_abort(
        "`log_lik` cannot return {unique(y[bad])}.",
        call = NULL
      ),
      NULL
    )
...
#' @noRd
vectorized_log_lik <- function(.fn, x, interface) {
  if (!is_double(x)) {
