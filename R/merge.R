...
merge.ernest_run <- function(x, y = NULL, ..., .keep = c("first", "all")) {
  if (!is.null(y)) {
    ...
    y <- list2(..., .named = TRUE)
    if (any(!vapply(y, \(yi) inherits(yi, "ernest_run"), logical(1)))) {
      cli::cli_abort(
        "All objects in `...` must be of class `ernest_run`."
      )
    }
  }
...
merge_rcrd <- function(
  ...
  list(
    "rcrd" = vctrs::vec_cast(out, new_ernest_rcrd()),
    "group_id" = data_frame0(
      "key" = c("x", "y"),
      "id" = c(list(vctrs::vec_unique(x_ids)), list(vctrs::vec_unique(y_ids)))
    )
  )
}
