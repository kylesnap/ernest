 ...
 */
[[cpp11::register]]
cpp11::list RandomWalkImpl(cpp11::writable::doubles original,
                           cpp11::function log_lik_fn, double criterion,
  ...
  for (size_t draw = 0; draw < steps; draw++) {
    lapack_wrapper::dcopy(original, next_draw);
    ...
    if (log_lik >= criterion) {
      lapack_wrapper::dcopy(next_draw, original);
      n_accept++;
    }
  }
  using namespace cpp11::literals;
  return cpp11::writable::list({"unit"_nm = original,
                                "log_lik"_nm = log_lik_fn(original),
