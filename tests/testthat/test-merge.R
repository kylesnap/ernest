...
test_that("merging two runs", {
  ...
  expect_identical(run3$nlive, run1$nlive + run2$nlive)
  expect_identical(length(unique(run3$weights$id)), run3$nlive)
  expect_false(is.unsorted(run3$weights$log_lik))
