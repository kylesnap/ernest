...
test_that("reindex_runs remaps IDs and preserves record fields", {
  ...
  n1 <- length(x1)
  n2 <- length(x2)
  nlive1 <- vctrs::vec_unique_count(field(x1, "id"))
  nlive2 <- vctrs::vec_unique_count(field(x2, "id"))

  expect_length(out, 2)
  expect_identical(sort(unique(field(out[[1]], "id"))), seq(nlive1))
  expect_identical(
    sort(unique(field(out[[2]], "id"))),
  ...
  expect_equal(
    ...
    min(
      field(run1$rcrd, "log_lik")[
        field(run1$rcrd, "evals") == 0
      ],
      field(run2$rcrd, "log_lik")[
        field(run2$rcrd, "evals") == 0
      ]
    )
  )
  expect_all_equal(field(res$live, "evals"), 0)
  expect_all_true(field(res$dead, "evals") > 0L)
  expect_identical(field(res$live, "id"), seq(nlive_total))
  expect_false(is.unsorted(field(res$dead, "log_lik")))
})
...
test_that("merging two runs", {
  ...
  expect_identical(sort(unique(field(run3_rcrd, "id"))), seq(run3$nlive))
  expect_length(field(run3_rcrd, "id"), run3$niter + 800)
  ...
  expect_identical(
    field(run3_rcrd, "log_lik")[1:run3$niter],
    field(run4_rcrd, "log_lik")[1:run3$niter]
  )
})
